...
class WF_Shipping_UPS_Admin
{
	...
	// European country
	private $eu_array = array('BE','BG','CZ','DK','DE','EE','IE','GR','ES','FR','HR','IT','CY','LV','LT','LU','HU','MT','NL','AT','PT','RO','SI','SK','FI','GB','PL','SE');
	...
	function wf_ups_metabox_content(){
		...
		if( empty( $created_shipments_details_array ) ) {		
			...
			if(empty($stored_packages)	&&	!is_array($stored_packages)){
				...
				?>
				<a class="button tips ups_generate_packages" href="<?php echo admin_url( '/?phupsgp='.base64_encode( $shipmentId.'|'.$post->ID ) ); ?>" data-tip="<?php _e( 'Generate Packages', 'ups-woocommerce-shipping' ); ?>"><?php _e( 'Generate Packages', 'ups-woocommerce-shipping' ); ?></a>
				<?php
				echo '</li>';
				?>
			...
			if(!empty($ups_label_details_array) && is_array($ups_label_details_array)){
				...
				foreach ( $created_shipments_details_array as $shipmentId => $created_shipments_details ){
					...
					if(isset($ups_commercial_invoice_details[$shipmentId])){
						...
						{
							echo '<a class="button button-primary tips" target="'.$target_val.'" href="'.admin_url( '/?wf_ups_print_commercial_invoice='.base64_encode($post->ID.'|'.$shipmentId)).'" data-tip="'.__('Downloads Commercial Invoice & NAFTA Certificate', 'ups-woocommerce-shipping').'">'.__('International Forms', 'ups-woocommerce-shipping').'</a><br><br>';		
						}else{
	...
	*/
		...
		if( is_array($shipment) )
		{
				...
				if( $this->billing_address_as_shipper ) {
					...
					$request_arr['Shipment']['Shipper']	=	array(
						'Name'			=>	substr( $billing_as_shipper['name'], 0, 34 ),
						'AttentionName'	=>	substr( $billing_as_shipper['company'], 0, 34 ),
						'Phone'			=>	array(
						...
						'Address'		=>	array(
							'AddressLine'		=>	substr( $billing_as_shipper['address'], 0, 34 ),
					...
					$request_arr['Shipment']['ShipFrom'] = array(
						...
						'Address'			=>	array(
							'AddressLine'		=>	substr( $from_address['address_1'], 0, 34 ),
					...
					$request_arr['Shipment']['Shipper']	=	array(
						'Name'			=>	substr( $from_address['name'], 0, 34 ),
						'AttentionName'	=>	substr( $from_address['company'], 0, 34 ),
						'Phone'			=>	array(
				...
				if(	$this->ship_from_address_different_from_shipper == 'yes' ) {
					...
					$ship_from_different_address = array(
						...
						'Address'		=>	array(
							'AddressLine'	=>	$different_ship_from_address['Address']['AddressLine1'],
				...
				if($this->wcsups->cod){
					...
					if( $this->wcsups->is_shipment_level_cod_required($to_address['country']) ){
						...
						if( array_key_exists( $to_address['country'], $this->cod_currency_specific_contries ) ) {
							$cod_currency = $this->cod_currency_specific_contries[ $to_address['country'] ];
							$cod_value = $this->get_currency_value( $cod_currency, $order->get_order_currency(), $cod_value);
						}
						else {
	...
	function wf_ups_shipment_confirmrequest( $order,$return_label=false ) {
			...
			foreach($shipments as $shipment) {
				...
				if( in_array($shipping_service,array_keys($this->freight_services)) )
				{
					...
					$request_arr['Shipment']=array();
						...
						if($return_label){
							$request_arr['Shipment']['ShipTo']	=	array(
								'CompanyName'	=>	substr( $from_address['name'], 0, 34 ),
								'AttentionName'	=>	substr( $from_address['company'], 0, 34 ),
								'PhoneNumber'	=>	preg_replace("/[^0-9]/", "", $from_address['phone']),
								'EMailAddress'	=>	$from_address['email'],
							);
						...
						if( $this->customandduties == 'shipper' || !empty($access_point_location) ) {
						...
						foreach ( $shipment['packages'] as $package ) {
							...
							if( $from_address['country'] == $to_address['country'] && in_array($from_address['country'],array('US','PR'))){
								...
								$description_value = $this->wf_get_shipment_description( $order );
						...
						if( $this->commercial_invoice && ( $from_address['country'] != $to_address['country'] ) && !(in_array($from_address['country'], $this->eu_array) && in_array($to_address['country'], $this->eu_array) ) ){ // Commercial Invoice is available only for international shipments
							...
							if($return_label){
								...
								$sold_to_arr	=	array(
									...
									'Address'		=>	array(
										'AddressLine1'	=>	substr( $billing_address['address_1'].' '.$billing_address['address_2'], 0, 34 ),
							...
							if( $this->nafta_co_form && !$return_label && isset($this->nafta_supported_countries[$ship_from_country]) && in_array($ship_to_country, $this->nafta_supported_countries[$ship_from_country]) )
							{
								...
								$shipmentServiceOptions['InternationalForms']['AdditionalDocumentIndicator'] = '';
		...
		*/
					...
					foreach ($package['Package']['items'] as $item_index => $item) {
						...
						else
						{
							...
							if( ( is_a( $product_data, 'WC_Product') || is_a( $product_data, 'wf_product') ) && $product_data->needs_shipping() ) {
								...
								$product_title =($product_id!=$item_id)?htmlspecialchars(strip_tags($product_data->get_formatted_name())):htmlspecialchars($product_data->get_title());
								...
								$product_details[$item_id]= array(
									'Description'	=>	$this->ph_get_commercial_invoice_description($product_id, $product_title),
									'Unit'			=>	array(
		...
		private function ph_get_commercial_invoice_description( $product_id, $invoice_default )
		{
		...
		if( "GIF" == $label_extn_code && $print_label_type == 'gif') {
			...
			if( "yes" == $show_label_in_browser ) {
				...
				if ( $label_format == 'laser_8_5_by_11' && !empty($html_image) ) {
					...
					echo $html_before_image."<img ".$image_style." src='data:image/gif;base64,".$graphic_image."'/>".$html_after_image;
				}
			...
			if( "yes" == $show_label_in_browser ) {
				...
				// 	if(empty($image_size_as_html)) {

				// 		$image_size_as_html = "width='793.92' height ='1122.24'";
				// 	}

				// 	echo "<img src='data:image/png;base64,".base64_encode($contents)."' ".$image_size_as_html." />";
	...
	function wf_get_shipping_service_data($order){
		...
		elseif( ! empty($shipping_service_tmp_data) && $shipping_service_tmp_data[0] == WF_UPS_ID && isset($shipping_service_tmp_data[1]) ) {			// Customer Selected Service if UPS
