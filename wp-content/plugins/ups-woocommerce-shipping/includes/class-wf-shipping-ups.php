...
?>
	...
	public function init_form_fields() {
		...
		$this->form_fields  = array(
			...
			'label_in_browser_zoom'  => array(
				...
				'desc_tip'		=> true,
				'class'			=> 'ph_ups_label_tab ups_display_browser_scale',
			),
			// 'resize_label'  => array(
			...
			'include_in_commercial_invoice' => array(
				'label'		   => __( 'Include Shipment Description for Commercial Invoice as well.<br/><br/><small><i>If disabled, Commercial Invoice will have Product Name as Default Description.</i></small>', 'ups-woocommerce-shipping' ),
				'type'			=> 'checkbox',
				'default'		 => 'no',
				'class'		=> 'ph_ups_label_tab'
			),
			...
			'freight_thirdparty_addressline'  => array(
				'title'			=> __( 'Address Line', 'ups-woocommerce-shipping' ),
				'type'			=> 'text',
				'description'	=> __( 'Third Party Address Line', 'ups-woocommerce-shipping' ),
				'default'		=> '',
				'desc_tip'		=> true,
				'class'			=>	'ph_ups_freight_tab ph_ups_freight_third_party_billing'
			),
	...
	public function calculate_shipping( $package=array() ) {
		...
		if( $this->address_validation && in_array( $package['destination']['country'], array( 'US', 'PR' ) ) && ! is_admin() ) {
	...
	public function  get_rate_requests_gfp( $package_requests, $package, $request_type='', $service_code='' ) {
			...
			$request['RateRequest']['Shipment']['Shipper']=array(
																'Address'=>array(
																					'AddressLine'=>$rate_request_data['origin_addressline'],
			...
			if( ! empty($rate_request_data['origin_state']) ) {
				...
				if( $this->ship_from_address_different_from_shipper == 'yes' && ! empty($rate_request_data['ship_from_addressline']) ) {
					...
					$request['RateRequest']['Shipment']['ShipFrom']=array(
																'Address'=>array(
																					'AddressLine'=>$rate_request_data['ship_from_addressline'],
				...
				if( isset($package['contents']) ){
					foreach( $package['contents'] as $product ) {
						...
						if($hazmat_product == 'yes'){
							...
							$req_arr['HazMatChemicalRecord']['UOM'] = $this->uom;
				...
				if( $this->show_est_delivery ) {
					...
					if( $this->origin_country != $package['destination']['country']) {
						...
						$request .= "\n<InvoiceLineTotal>
											...
											<MonetaryValue>".round(($package['contents_cost'] / (float)$this->conversion_rate),2)."</MonetaryValue>
										</InvoiceLineTotal>\n";
					}
				}
				$request .= "	</Shipment>" . "\n";
				...
				if( $this->insuredvalue || $this->cod ) {
					...
					if($this->cod){
						if( ! $this->is_shipment_level_cod_required($this->destination['country']) ){
							...
							if( array_key_exists( $this->destination['country'], $this->cod_currency_specific_contries ) ) {
								$cod_currency = $this->cod_currency_specific_contries[ $this->destination['country'] ];

								$shipping_admin_object = new WF_Shipping_UPS_Admin();
								$cod_value = $shipping_admin_object->get_currency_value( $cod_currency, $this->order->get_order_currency(), $cod_value);
							}
							else {
								$cod_currency = $this->order->get_order_currency();
							}
							$request['Package']['PackageServiceOptions']['COD']	=	array(
								...
								'CODAmount'		=>	array(
									...
									'MonetaryValue'	=>	(string) round( $cod_value, 2),
								),
							);
						}
					}
