...
Class wf_freight_ups{
	...
	function get_rate_request( $package, $code, $ups_packages )
	{
		...
		if( $this->parent->freight_payment_information == '10' )
		{
			...
			$payor_addressline  = $this->parent->origin_addressline;
			...
			$payor_addressline  = $this->parent->freight_thirdparty_addressline;
		...
		if( is_array($ups_packages) ) {
			...
			if( ! empty($weight) ) {
					$commodity	=	array(
							...
							'Weight'	=>array(
									'Value'				=> (string) $weight,
									'UnitOfMeasurement'	=> array('Code'=>(string) $ups_packages[0]['Package']['PackageWeight']['UnitOfMeasurement']['Code'] )
							),
					);
					...
					if( ! empty($ups_packages[0]['Package']['Dimensions']) ) {
						$commodity['Dimensions']	=	array(
								'UnitOfMeasurement'	=>	array(
										'Code'	=>	(string)$ups_packages[0]['Package']['Dimensions']['UnitOfMeasurement']['Code']
								),
								'Length'	=>	(string) round($ups_packages[0]['Package']['Dimensions']['Length'], 2),
								'Width'		=>	(string) round($ups_packages[0]['Package']['Dimensions']['Width'], 2),
								'Height'	=>	(string) round($ups_packages[0]['Package']['Dimensions']['Height'], 2)
						);
					}
				...
				"Shipment"	=> array(
					   ...
					   "PaymentInformation"=>array(
								"Payer"=>array(
										...
										'Address'		=>	array(
												'AddressLine'			=>	htmlspecialchars( $payor_address['address_1'] ),
					   ...
					   "Service"=>array(
						...
						// 		"NegotiatedRatesIndicator"	=> "0"				// Node doesn't exist
						// ),
					   "HandlingUnitOne"=>array(
								"Quantity"	=> "1",
								"Type"		=> array(
