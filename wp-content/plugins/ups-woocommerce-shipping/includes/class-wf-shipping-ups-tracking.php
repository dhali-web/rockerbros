...
class WF_Shipping_UPS_Tracking
{
	...
	function wf_add_ups_tracking_info_to_email( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if( $shipment_id_cs != '' ) {
			...
			$ups_setting 			= get_option('woocommerce_wf_shipping_ups_settings');
			$custom_message 		= ! empty($ups_setting['custom_message']) ? $ups_setting['custom_message'] : __( 'Your order is shipped via UPS. To track your shipment, please follow the Tracking Number(s) ', 'ups-woocommerce-shipping' );
			$return_custom_message 	= ! empty($ups_setting['custom_message']) ? $ups_setting['custom_message'] : __( 'In case you use the return labels, then to track the return shipments please follow the Tracking Number(s) ', 'ups-woocommerce-shipping' );
			...
			if( !empty($return_order_number) )
			{
				...
				echo '<p>'.$return_custom_message . $return_order_number.'</p></br>';
			}
		}
	}
