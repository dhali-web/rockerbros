...
class Support {
	...
	public function envira_edit_gallery_settings() {
		...
		foreach ( $oringial_settings['config'] as $oringial_setting => $oringial_value ) {
			...
			if ( property_exists( 'value-' . $oringial_setting, $updated_settings ) && $updated_settings[ 'value-' . $oringial_setting ] !== $oringial_value ) {
	...
	public function get_problem_galleries( $image_limit = 100 ) {
		...
		foreach ( $galleries as $gallery ) {
			...
			$settings = json_decode( envira_get_gallery_config( $gallery_id ) );
			if ( property_exists( 'proofing_lightbox', $settings ) && ! class_exists( 'Envira_Proofing' ) ) {
				$problem_galleries[] = array(
					'link'       => admin_url( 'post.php?post=' . $gallery['id'] . '&action=edit' ),
					'title'      => ( 'Gallery #' . $gallery['id'] ),
			...
			if ( property_exists( 'custom_css', $settings ) && ! defined( 'ENVIRA_CUSTOM_CSS_PLUGIN_NAME' ) ) {
				$problem_galleries[] = array(
					'link'       => admin_url( 'post.php?post=' . $gallery['id'] . '&action=edit' ),
					'title'      => ( 'Gallery #' . $gallery['id'] ),
			...
			if ( ( property_exists( 'exif', $settings ) || property_exists( 'exif_lightbox', $settings ) ) && ! class_exists( 'Envira_Exif' ) ) {
				$problem_galleries[] = array(
					'link'       => admin_url( 'post.php?post=' . $gallery['id'] . '&action=edit' ),
					'title'      => ( 'Gallery #' . $gallery['id'] ),
			...
			if ( ( property_exists( 'pagination_images_per_page', $settings ) && 1 === $settings->pagination && ! class_exists( 'Envira_Pagination' ) ) ) {
				$problem_galleries[] = array(
					'link'       => admin_url( 'post.php?post=' . $gallery['id'] . '&action=edit' ),
					'title'      => ( 'Gallery #' . $gallery['id'] ),
			...
			if ( property_exists( 'pagination_images_per_page', $settings ) && 1 === $settings->pagination && 0 === $settings->pagination_images_per_page && class_exists( 'Envira_Pagination' ) ) {
				$problem_galleries[] = array(
					'severity'   => 'warning',
					'link'       => admin_url( 'post.php?post=' . $gallery['id'] . '&action=edit' ),
					'title'      => ( 'Gallery #' . $gallery['id'] ),
